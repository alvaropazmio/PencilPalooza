import { Behaviour, GameObject, Gizmos, NEPointerEvent, getTempVector, serializable } from "@needle-tools/engine";
...
import { LineHandle, LinesManagerPencil } from "./LinesManagerPencil";
...
export class LinesDrawerPencil extends Behaviour {

    //@type LinesManagerPencil
    @serializable(LinesManagerPencil)
    lines!: LinesManagerPencil;
    ...
    start() {
        if (!this.lines) {
            this.lines = GameObject.getComponent(this.gameObject, LinesManagerPencil)!;
            if (!this.lines)
                this.lines = GameObject.addNewComponent(this.gameObject, LinesManagerPencil);
        }
    ...
    private createRandomMaterial() {
        ...
        return new MeshLineMaterial({
            color: col,
            lineWidth: Mathf.lerp(0.03, 0.05, Math.random()),
        });
    }
}
